...
private:
    VectorType* data_ptr = nullptr;  // Pointer to vector data (internal or external)
...
public:
     ...
     */
    VectorFile(const char* filename, FileMode mode) : data_ptr(&internal_data), name_(filename) {
    ...
    bool open(FileMode mode) {
        ESP_LOGD(TAG, "Opening file '%s' with mode %d", name_.c_str(), static_cast<int>(mode));
        this->mode = mode;
    ...
    int read() override {
        ...
        if (position_ >= data_ptr->size()) {
            ESP_LOGD(TAG, "read at EOF: pos=%u, size=%u", (unsigned)position_, (unsigned)data_ptr->size());
            return -1;  // EOF
        }
    ...
    size_t write(uint8_t b) override {
        ...
        if (!open_ || (mode != FileMode::WRITE && mode != FileMode::APPEND && mode != FileMode::READ_WRITE)) {
